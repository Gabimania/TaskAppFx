...
public class Task extends ModeloBase{
    ...
    public List<Task> getAllByUser(int iduser) {
        ...
        Task task1 = new Task();
        Connection con = task1.getConnection();
    ...
    public List<Task> getAllTasks() {
       ...
       Task task1= new Task();
        Connection con = task1.getConnection();
        String sql = " select idtask, title, T0.description, create_date, deadline, status, T1.iduser, username,T2.idrol, T2.description as rol  from task T0  left join user T1 on T0.iduser= T1.iduser  left join rol T2 on T1.idrol = T2.idrol;";
        try {
